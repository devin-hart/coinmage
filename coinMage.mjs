...
const getSparkline = async (coinId) => {
    try {
      ...
      console.error(chalk.red(`Failed to fetch sparkline: ${err.message}`));
      return null;
    }
  };
...
const getCoinData = async (coinId) => {
    ...
    const formatPriceLabel = (x) => {
        ...
        // Take first 4 non-zero digits after the leading zeros
        const meaningful = decimal.slice(leadingZeros).slice(0, 4);
      
        return `$${whole}.${'0'.repeat(leadingZeros)}${meaningful}`;
    };
    try {
        ...
        const spark = await getSparkline(coinId);
        if (spark) {
            console.log(chalk.cyanBright('\n7-Day Price Trend:\n'));
            console.log(asciichart.plot(spark.slice(-100), {
                height: 10,
                padding: '       ',
